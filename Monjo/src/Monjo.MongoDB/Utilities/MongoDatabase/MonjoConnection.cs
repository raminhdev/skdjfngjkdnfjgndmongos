...
{
    /// <summary>
    /// The original Mongo connection class, kept for compatibility. NOT registered by Monjo DI —
    /// <see cref="Monjo.MongoDB.MongoMonjoConnection"/> (registered by <c>UseMonjoMongoDB</c>)
    /// provides the same <see cref="IMonjoConnection"/> contract with the same client settings.
    /// </summary>
    [System.Obsolete("Use the provider connection registered by services.UseMonjoMongoDB(). Kept for compatibility.")]
    public class MonjoConnection : IMonjoConnection
    {
